...
import { addDaemonSetYamlData } from '@/api/containerApi';
...
export default function DaemonSetCreate() {
  ...
  const [daemonSetYaml, setDaemonSetYaml] = useState(daemonSetYamlExample);
  ...
  const handleSaveYaml = async () => {
    ...
    if (!errorCount) {
      ...
      if (passNamespace) {
        try {
          const res = await addDaemonSetYamlData(passNamespace, yamlJson);
        ...
        } catch (damonSetCreateError) {
          if (damonSetCreateError.response.status === ResponseCode.Forbidden) {
            forbiddenMsg(messageApi, damonSetCreateError);
          } else {
            messageApi.error(`创建失败！${damonSetCreateError.response.data.message}`);
          }
        }
      } else {
  ...
  const handleResetCode = () => {
    setDaemonSetYaml(daemonSetYamlExample);
    childCodeMirrorRef.current.resetCodeEditor(daemonSetYamlExample);
  };

  const exportYaml = () => {
    exportYamlOutPut('DaemonSet', daemonSetYaml);
  ...
  return <div className="child_content">
    ...
    <div className="tab_container container_margin_box normal_container_height">
      ...
      <div className="yaml_space_box">
        <CodeMirrorEditor yamlData={daemonSetYaml} changeYaml={handleChangeYaml} ref={childCodeMirrorRef} />
