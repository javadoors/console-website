...
import { updateDaemonSetYamlData } from '@/api/containerApi';
import { yamlTojson, exportYamlOutPut, forbiddenMsg } from '@/tools/utils';
export default function DaemonSetDetailYaml({ daemonSetYamlProps, readOnly, handleEditFn, refreshFn }) {
  const themeStore = useStore('theme');
  const [daemonSetYaml, setDaemonSetYaml] = useState(daemonSetYamlProps);
  ...
  const handleSaveYaml = async () => {
    ...
    let passName = '';
    try {
      yamlJson = yamlTojson(daemonSetYaml);
      passNamespace = yamlTojson(daemonSetYamlProps).metadata.namespace; // 默认取之前的命名空间
    ...
    if (!errorCount) {
      if (passNamespace) {
        try {
          const res = await updateDaemonSetYamlData(passNamespace, passName, yamlJson);
  ...
  const handleResetCode = () => {
    setDaemonSetYaml(daemonSetYamlProps);
    childCodeMirrorRef.current.resetCodeEditor(daemonSetYamlProps);
  };

  const exportYaml = () => {
    exportYamlOutPut(`${fileName}`, daemonSetYaml);
  ...
  useEffect(() => {
    const detailData = yamlTojson(daemonSetYamlProps);
  ...
  return <div className="tab_container container_margin_box normal_container_height">
    ...
    <div className="yaml_space_box">
      <CodeMirrorEditor className={readOnly ? 'no_btn' : ''} yamlData={daemonSetYaml} changeYaml={handleChangeYaml} isEdit={!readOnly} ref={childCodeMirrorRef} />
