...
import { addPodYamlData } from '@/api/containerApi';
...
export default function PodCreate() {
  ...
  const [podYaml, setPodYaml] = useState(podYamlExample);
  ...
  const handleSaveYaml = async () => {
    ...
    if (!errorCount) {
      ...
      if (passNamespace) {
        try {
          const res = await addPodYamlData(passNamespace, yamlJson);
        ...
        } catch (podCreateError) {
          if (podCreateError.response.status === ResponseCode.Forbidden) {
            forbiddenMsg(messageApi, podCreateError);
          } else {
            messageApi.error(`创建失败！${podCreateError.response.data.message}`);
          }
        }
      } else {
  ...
  const handleResetCode = () => {
    setPodYaml(podYamlExample);
    childCodeMirrorRef.current.resetCodeEditor(podYamlExample);
  };

  const exportYaml = () => {
    exportYamlOutPut('Pod', podYaml);
  ...
  return <div className="child_content">
    ...
    <div className="tab_container container_margin_box normal_container_height">
      ...
      <div className="yaml_space_box">
        <CodeMirrorEditor yamlData={podYaml} changeYaml={handleChangeYaml} ref={childCodeMirrorRef} />
