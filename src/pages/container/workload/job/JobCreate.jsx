...
import { addJobYamlData } from '@/api/containerApi';
...
export default function JobCreate() {
  ...
  const [jobYaml, setJobYaml] = useState(jobYamlExample);
  ...
  const handleSaveYaml = async () => {
    ...
    if (!errorCount) {
      ...
      if (passNamespace) {
        try {
          const res = await addJobYamlData(passNamespace, yamlJson);
          ...
          if (jobCreateError.response.status === ResponseCode.Forbidden) {
            forbiddenMsg(messageApi, jobCreateError);
          } else {
            messageApi.error(`创建失败！${jobCreateError.response.data.message}`);
          }
        }
      } else {
  ...
  const handleResetCode = () => {
    setJobYaml(jobYamlExample);
    childCodeMirrorRef.current.resetCodeEditor(jobYamlExample);
  };

  const exportYaml = () => {
    exportYamlOutPut('Job', jobYaml);
  ...
  return <div className="child_content">
    ...
    <div className="tab_container container_margin_box normal_container_height">
      ...
      <div className="yaml_space_box">
        <CodeMirrorEditor yamlData={jobYaml} changeYaml={handleChangeYaml} ref={childCodeMirrorRef} />
