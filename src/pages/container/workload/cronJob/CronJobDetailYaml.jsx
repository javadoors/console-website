...
import { updateCronJobYamlData } from '@/api/containerApi';
import { yamlTojson, exportYamlOutPut, forbiddenMsg } from '@/tools/utils';
export default function CronJobDetailYaml({ cronJobYamlProps, readOnly, handleEditFn, refreshFn }) {
  const themeStore = useStore('theme');
  const [cronJobYaml, setCronJobYaml] = useState(cronJobYamlProps);
  ...
  const handleSaveYaml = async () => {
    ...
    try {
      ...
      passNamespace = yamlTojson(cronJobYamlProps).metadata.namespace; // 默认取之前的命名空间
    ...
    if (!errorCount) {
      if (passNamespace) {
        try {
          const res = await updateCronJobYamlData(passNamespace, passName, yamlJson);
  ...
  const handleResetCode = () => {
    setCronJobYaml(cronJobYamlProps);
    childCodeMirrorRef.current.resetCodeEditor(cronJobYamlProps);
  };
  ...
  const exportYaml = () => {
    exportYamlOutPut(`${fileName}`, cronJobYaml);
    messageApi.success('导出成功');
  };

  useEffect(() => {
    const detailData = yamlTojson(cronJobYamlProps);
  ...
  return <div className="tab_container container_margin_box normal_container_height">
    ...
    <div className="yaml_space_box">
      <CodeMirrorEditor className={readOnly ? 'no_btn' : ''} yamlData={cronJobYaml} changeYaml={handleChangeYaml} isEdit={!readOnly} ref={childCodeMirrorRef} />
