...
import { updateStatefulSetYamlData } from '@/api/containerApi';
import CodeMirrorEditor from '@/components/CodeMirrorEditor';
import { yamlTojson, exportYamlOutPut, forbiddenMsg } from '@/tools/utils';
export default function StatefulSetDetailYaml({ statefulSetYamlProps, readOnly, handleEditFn, refreshFn }) {
  const themeStore = useStore('theme');
  const [statefulSetYaml, setStatefulSetYaml] = useState(statefulSetYamlProps);
  ...
  const handleSaveYaml = async () => {
    ...
    try {
      ...
      passNamespace = yamlTojson(statefulSetYamlProps).metadata.namespace; // 默认取之前的命名空间
    ...
    if (!errorCount) {
      if (passNamespace) {
        try {
          const res = await updateStatefulSetYamlData(passNamespace, passName, yamlJson);
  ...
  const handleResetCode = () => {
    setStatefulSetYaml(statefulSetYamlProps);
    childCodeMirrorRef.current.resetCodeEditor(statefulSetYamlProps);
  };

  const exportYaml = () => {
    exportYamlOutPut(fileName, statefulSetYaml);
  ...
  useEffect(() => {
    const detailData = yamlTojson(statefulSetYamlProps);
    setFileName(detailData.metadata.name);
  }, [statefulSetYamlProps]);
  ...
  return <div className="tab_container container_margin_box normal_container_height">
    ...
    <div className="yaml_space_box">
      <CodeMirrorEditor className={readOnly ? 'no_btn' : ''} yamlData={statefulSetYaml} changeYaml={handleChangeYaml} isEdit={!readOnly} ref={childCodeMirrorRef} />
