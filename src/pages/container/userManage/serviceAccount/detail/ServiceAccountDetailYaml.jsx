...
export default function ServiceAccountDetailYaml({ serviceAccountYamlProps, readOnly, handleEditFn, refreshFn }) {
  const [serviceAccountYaml, setServiceAccountYaml] = useState(serviceAccountYamlProps);
  ...
  const exportYaml = () => {
    exportYamlOutPut(`${fileName}`, serviceAccountYaml);
  ...
  const handleSaveYaml = async () => {
    ...
    try {
      ...
      passNamespace = yamlTojson(serviceAccountYamlProps).metadata.namespace; // 默认取之前的命名空间
    ...
    if (!errorCount) {
      if (passNamespace) {
        try {
          const res = await updateServiceAccountYamlData(passNamespace, passName, yamlJson);
  ...
  const handleResetCode = () => {
    setServiceAccountYaml(serviceAccountYamlProps);
    childCodeMirrorRef.current.resetCodeEditor(serviceAccountYamlProps);
  };
  ...
  useEffect(() => {
    const detailData = yamlTojson(serviceAccountYamlProps);
      ...
      <div className="yaml_flex_box">
        ...
        <div className="yaml_tools">
          <div className="tool_word_group" onClick={exportYaml}>
            <ExportOutlined className="common_antd_icon primary_color ServiceAccountDetailYaml" />
            <span>导出</span>
          </div>
          <div className="tool_word_group" onClick={handleCopyYaml}>
            <CopyOutlined className="common_antd_icon primary_color ServiceAccountDetailYaml" />
            <span>复制</span>
          </div>
        </div>
      </div>
    </div>
    <div className='yaml_space_box ServiceAccountDetailYaml'>
      <CodeMirrorEditor className={readOnly ? 'no_btn' : ''} yamlData={serviceAccountYaml} changeYaml={handleChangeYaml} isEdit={!readOnly} ref={childCodeMirrorRef} />
