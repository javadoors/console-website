...
export default function ServiceAccountCreate() {
  ...
  const [serviceAccountYaml, setServiceAccountYaml] = useState(serviceAccountYamlExample);
  ...
  const handleSaveYaml = async () => {
    ...
    if (!errorCount) {
      ...
      if (passNamespace) {
        try {
          const res = await addServiceAccountsYamlData(passNamespace, yamlJson);
        ...
        } catch (serviceAccountCreateError) {
          if (serviceAccountCreateError.response.status === ResponseCode.Forbidden) {
            forbiddenMsg(messageApi, serviceAccountCreateError);
          } else {
            messageApi.error(`创建失败！${serviceAccountCreateError.response.data.message}`);
          }
        }
      } else {
  ...
  const handleResetCode = () => {
    setServiceAccountYaml(serviceAccountYamlExample);
    childCodeMirrorRef.current.resetCodeEditor(serviceAccountYamlExample);
  };

  const exportYaml = () => {
    exportYamlOutPut('ServiceAccount', serviceAccountYaml);
  ...
  return <div className="child_content withBread_content">
    ...
    <BreadCrumbCom className="create_bread" items={[
      ...
      { title: '服务账号' },
      { title: '创建', path: `/createServiceAccount` },
    ]} />
    <div className="tab_container container_margin_box normal_container_height">
      ...
      <div className="yaml_space_box">
        <CodeMirrorEditor yamlData={serviceAccountYaml} changeYaml={handleChangeYaml} ref={childCodeMirrorRef} />
